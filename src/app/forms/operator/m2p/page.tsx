...
interface ModuleToPackFormData {
  govde_temizligi: string;
  sizdirmazlik_verisi: string;
  cikis_konnektor_montaji: string;
}

const ModuleToPackFormData: React.FC = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<ModuleToPackFormData>();
...
//  console.log("Rendering PackLoadForm for user:", userInfo.firstName, userInfo.lastName);
  ...
  const onSubmit = (data: ModuleToPackFormData) => {
    const formData = {
      ...data,
...
export default ModuleToPackFormData;
